...
void FileManagerWidgetTest::testDefaultValues()
{
    ...
    QCOMPARE(widget.url(), QUrl::fromLocalFile(QDir::homePath()));
    ...
    QCOMPARE(widget.selectedUrls(), QList<QUrl>());
...
void FileManagerWidgetTest::testState()
{
    ...
    widget.setUrl(QUrl::fromLocalFile(QDir::homePath()));
    ...
    QCOMPARE(widget2.url(), widget.url());
